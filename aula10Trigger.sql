...
CREATE OR REPLACE FUNCTION A10.UPDATE_PARTIDA()
RETURNS TRIGGER AS $$
BEGIN
    INSERT INTO A10.LOG_PARTIDA(PARTIDA_ID, ACAO) VALUES (NEW.ID, 'UPDATE');
    RETURN NEW;
END;
$$ LANGUAGE PLPGSQL;

CREATE OR REPLACE TRIGGER UPDATE_PARTIDA
BEFORE UPDATE ON A10.PARTIDA
FOR EACH ROW
EXECUTE FUNCTION A10.UPDATE_PARTIDA();

UPDATE A10.partida SET time_2_gols = 1 WHERE ID = 11;

SELECT * FROM A10.LOG_PARTIDA;
SELECT * FROM A10.PARTIDA;

-- Fazer uma trigger que impressa de fazer update em partidas que já foram finalizados

CREATE OR REPLACE FUNCTION A10.update_partida()
RETURNS TRIGGER AS $$
BEGIN
    IF new.TIME_1_GOLS IS NULL AND NEW.TIME_2_GOLS IS NULL THEN
        INSERT INTO A10.LOG_PARTIDA(PARTIDA_ID, ACAO) VALUES (NEW.ID, 'UPDATE');
        RETURN NEW;
    ELSE
        RAISE EXCEPTION 'Não pode inserir valores de gols em partidas já finalizadas';
    END IF;
END;
$$ LANGUAGE PLPGSQL; 

CREATE OR REPLACE FUNCTION A10.DELETE_PARTIDA()
RETURNS TRIGGER AS $$
BEGIN
    -- INSERT INTO A10.LOG_PARTIDA(PARTIDA_ID, ACAO) VALUES (OLD.ID, 'DELETE');
    -- RETURN OLD;
    RAISE EXCEPTION 'NÃO É PERMITIDO DELETAR PARTIDA';
END;
$$ LANGUAGE PLPGSQL;

CREATE TRIGGER DELETE_PARTIDA
BEFORE DELETE ON A10.partida
FOR EACH ROW
EXECUTE FUNCTION A10.delete_partida();

DELETE FROM A10.partida WHERE ID = 16;
